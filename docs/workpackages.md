...
- Add a fixed deploy wrapper that requires the workflow triggering SHA, ignores
  `$GITHUB_WORKSPACE`, fetches the protected upstream branch into a clean
  deploy checkout, binds deploy to the workflow triggering SHA, and applies
  Ansible through local connection with a scrubbed environment.
- Require an explicit repository visibility acknowledgement; private repository
  is the intended enablement posture for the persistent runner pilot.
...
- The deploy workflow runs only after protected-branch merge and Environment
  approval.
- The persistent runner is enabled only for a private repo or an explicit
  public-repo pilot risk waiver.
- The deploy wrapper fails if the branch head no longer matches the triggering
  SHA.
