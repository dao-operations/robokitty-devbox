...
vault_robokitty_deploy_runner_repo_visibility_ack: private-repo
...
Runner auto-update is disabled at registration; update the runner by changing
the pinned archive URL/checksum and applying Ansible.
...
- make the infra repository private before enablement, or set
  `vault_robokitty_deploy_runner_repo_visibility_ack:
  public-repo-risk-accepted` only for a deliberately time-boxed pilot,
...
The current pilot still uses a persistent self-hosted runner whose job user
owns its runner installation; a private repository reduces exposure but does
not replace the GitHub controls above.
...
2. Remove the self-hosted runner from GitHub repository settings.
3. Set `vault_robokitty_deploy_runner_enabled: false`.
4. Set `vault_robokitty_deploy_runner_github_removed_ack:
   github-runner-removed`.
5. Apply Ansible from the operator machine.
