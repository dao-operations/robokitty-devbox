...
`host-resident-vault-password` acknowledgement, and the vault password value
that Ansible writes to the `agent-deploy` account. This is a deliberate
departure from human-only vault custody; read
