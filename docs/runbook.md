...
protected `master` branch, never on pull request or fork events.
...
5. The workflow runs `sudo -n /usr/local/bin/robokitty-deploy-infra`.
6. The wrapper deploys the reviewed upstream `master` branch from its own clean
   checkout.
