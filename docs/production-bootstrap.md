...
Do not put the Ansible Vault password in this repo. Prefer storing the operator
SSH private key in a password manager, not in this repository, even if
encrypted.
...
protected-branch, Environment, and runner-scope controls are configured, an
explicit repository visibility acknowledgement, and the vault password value
that Ansible writes to the `agent-deploy` account. Prefer making the infra repo
private before enablement; `public-repo-risk-accepted` is only a time-boxed
pilot waiver. This is a deliberate departure from human-only vault custody; read
